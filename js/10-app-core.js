    ...
    try {
        ...
        filtered.forEach(function(p) {
            if (isGrouped) {
                ...
                if (curDept !== lastDept) {
                    lastDept = curDept;
                    bodyHTML += '<tr style="background:linear-gradient(90deg,#dbeafe,#eff6ff);"><td colspan="8" style="padding:4px 10px;font-weight:800;font-size:13px;color:#1d4ed8;border-bottom:2px solid #bfdbfe;"><i class="fa-solid fa-layer-group" style="margin-right:5px;"></i>' + curDept + '</td></tr>';
                }
            }
            ...
            bodyHTML += '<tr ' + rowBg + '>' +
                ...
                '<td style="padding:4px 8px;text-align:center;">' +
                    '<select onchange="updateImproveField(\'' + p.id + '\',\'audit\',this.value)" style="width:100%;font-size:12px;font-weight:700;cursor:pointer;border:none;background:transparent;padding:2px 4px;border-radius:4px;">' +
                        '<option value="" ' + (!p.audit ? 'selected' : '') + '>--</option>' +
                        '<option value="NG" ' + (p.audit === 'NG' ? 'selected' : '') + ' style="color:#dc2626;font-weight:900;">NG</option>' +
                        '<option value="OK" ' + (p.audit === 'OK' ? 'selected' : '') + ' style="color:#16a34a;font-weight:900;">OK</option>' +
                    '</select>' +
                '</td>' +
        ...
        bodyHTML = bodyHTML || '<tr><td colspan="8" style="text-align:center;padding:30px;color:#94a3b8;font-size:14px;"><i class="fa-solid fa-inbox" style="font-size:24px;display:block;margin-bottom:8px;"></i>' + t('improve_empty') + '</td></tr>';
...
window.generateImprovePoster = function() {
    try {
        ...
        deptRanking.forEach(function(d, idx) {
            ...
            if (d.notstart > 0) h += '<span class="ip-brk-not">\u26AA' + d.notstart + '</span> ';
            h += '</div>';
            h += '<div class="ip-chart-total">' + d.total + '</div>';
            h += '</div>';
        });
        h += '</div></div>';
        ...
        function renderDeptGroups(groups) {
            ...
            for (var gi = 0; gi < groups.length; gi++) {
                ...
                html += '<span class="ip-sub-hdr-audit">Audit</span>';
                html += '</div>';
                ...
                for (var pi = 0; pi < g.items.length; pi++) {
                    ...
                    html += '<span class="ip-item-audit" style="white-space:nowrap;text-align:center;">' +
                        (p.audit === 'NG' ? '<span style="color:#dc2626;font-weight:900;font-size:10px;background:#fef2f2;padding:1px 6px;border-radius:3px;border:1px solid #fecaca;">NG</span>' :
                        (p.audit === 'OK' ? '<span style="color:#16a34a;font-weight:900;font-size:10px;background:#f0fdf4;padding:1px 6px;border-radius:3px;border:1px solid #bbf7d0;">OK</span>' :
        ...
        h += '<div class="ip-col">' + renderDeptGroups(leftGroups) + '</div>';
        h += '<div class="ip-col">' + renderDeptGroups(rightGroups) + '</div>';
        h += '</div></div>';
        ...
        var inlineCSS = '<style>' +
            ...
            '.pwrap .ip-sub-hdr-audit{font-size:9px;font-weight:700;color:#64748b;min-width:50px;text-align:center;}' +
            // ITEM ROW
            ...
            '.pwrap .ip-item-audit{min-width:50px;text-align:center;font-size:10px;overflow:hidden;text-overflow:ellipsis;}' +
