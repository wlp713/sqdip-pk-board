        ...
        window.aiAnalyzeSystemRepeat = async function() {
            ...
            const prompt = '你是GMCC（美的冰箱压缩机）泰国工厂GAT的精益数据分析师。产品涵盖往复式/旋转式冰箱压缩机，制造流程包括定子冲压→转子压铸→定子绕线→曲轴箱精密加工→装配（活塞/阀片/连杆）→焊接密封→氦检漏→性能测试→包装入库。MBS工具：标准作业(SW)、TPM、安灯(Andon)、SMED、PSP/PDCA闭环、DM管理、SQDIP、LOSS reduction、VSM。\n\n'
                + promptData
...
三、SQDIP各维度数据摘要：
   安全/质量/交付/库存/效率各维度的关键数据点

四、管理提示（仅10%）：1-2句基于数据的关注方向
